...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const removeOldImages = () => {
    ...
    if (now - lastRemovalTime < config.removalDelay || trail.length === 0)
      return; // Skip if too soon or no images
  ...
  const handleFormSubmit = (e) => {
    ...
    // Replace the placeholder below with your actual email address.
    // Example: const formSubmitEndpoint = 'https://formsubmit.co/ajax/youremail@example.com';
    const formSubmitEndpoint =
      "https://formsubmit.co/ajax/itsurmanishmehra@gmail.com";
    ...
    fetch(formSubmitEndpoint, {
      ...
      .then((data) => {
        if (data.success === "true" || data.success === true) {
          ...
          // If FormSubmit returns an error, open mail client as fallback
          window.location.href = `mailto:${encodeURIComponent("itsurmanishmehra@gmail.com")}?subject=${encodeURIComponent("Contact Form Submission")}`;
        }
      })
      .catch(() => {
        // Network error fallback: open mail clientf
        window.location.href = `mailto:${encodeURIComponent("itsurmanishmehra@gmail.com")}?subject=${encodeURIComponent("Contact Form Submission")}`;
      })
      .finally(() => {
